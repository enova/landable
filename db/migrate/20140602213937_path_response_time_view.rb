class PathResponseTimeView < ActiveRecord::Migration
  def up
	...
	CREATE OR REPLACE VIEW "#{Landable.configuration.database_schema_prefix}_landable_traffic.path_response_time__by_time"
	AS
	SELECT * 
	FROM 
	(
	...
	COMMENT ON VIEW "#{Landable.configuration.database_schema_prefix}_landable_traffic.path_response_time__by_time" IS $$ The view path_response_time__by_time returns the paths displayed and the average reponse time (in ms) for each page of the site.  This list is ordered by response time, decending.$$;
  SQL
  end
end
