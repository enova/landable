...
SELECT throws_matching($$INSERT INTO landable.page_revisions(ordinal, page_id, author_id, snapshot_attributes) SELECT 1, page_id, author_id, 'Test attrs' FROM landable.pages, landable.authors$$, 'ordinal');
...
--Verify cannot update fields others than is_published, updated_at, snapshot_attributes for page_revisions
...
SELECT lives_ok($$UPDATE landable.page_revisions SET snapshot_attributes = 'Updated'$$, 'Can update snapshot_attributes');
